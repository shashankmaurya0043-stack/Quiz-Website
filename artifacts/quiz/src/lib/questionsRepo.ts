...
export function groupBySubject(list: QuestionDoc[]): Record<string, QuestionRaw[]> {
...
export async function getActiveQuestions(): Promise<{ bySubject: Record<string, QuestionRaw[]>; source: "api" | "static"; total: number; }> {
  try {
    ...
    console.warn("API fetch failed, falling back to static.");
  }
...
export async function updateQuestion(id: string, patch: NewQuestionInput): Promise<QuestionDoc> {
...
export async function seedStaticQuestions(onProgress?: (done: number, total: number) => void): Promise<number> {
